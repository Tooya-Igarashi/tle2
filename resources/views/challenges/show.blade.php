<x-app-layout>

    {{-- Header --}}
    <header role="banner">
        <x-slot name="header">
            {{ $challenge->title }}
        </x-slot>
    </header>

    {{-- Hoofdinhoud --}}
    <main id="main" role="main" class="bg-white py-8 px-4 pb-20">
        <div class="max-w-3xl mx-auto">

            {{-- Challenge info --}}
            <section class="rounded-2xl shadow-md p-6 md:p-8 bg-green-200"
                     aria-labelledby="challenge-heading">

                <div class="flex flex-col md:flex-row gap-6">

                    <img
                        src="{{ asset('storage/' . $challenge->image_path) }}"
                        alt="Afbeelding bij de uitdaging {{ $challenge->title }}"
                        class="w-44 h-auto object-cover rounded-xl"
                    >

                    <div class="flex-1">

                        <h2 id="challenge-heading"
                            class="text-3xl font-bold text-gray-900 mb-2">
                            {{ $challenge->title }}
                        </h2>

                        {{-- Moeilijkheidsgraad --}}
                        @php
                            $diffId = optional($challenge->difficulty)->id ?? 0;

                            $labels = [
                                1 => 'Makkelijk',
                                2 => 'Gemiddeld',
                                3 => 'Moeilijk',
                            ];

                            $stars = $diffId;
                            $difficultyLabel = $labels[$diffId] ?? 'Onbekend';
                        @endphp

                        <div class="flex items-center gap-1 mb-3"
                             aria-label="Moeilijkheidsgraad: {{ $difficultyLabel }}, {{ $stars }} van de 3 sterren">

                            @for ($i = 1; $i <= 3; $i++)
                                <span
                                    class="text-xl {{ $i <= $stars ? 'text-yellow-400' : 'text-gray-300' }}"
                                    aria-hidden="true">
                                    ★
                                </span>
                            @endfor

                            <span class="ml-2 text-sm text-gray-800 font-bold">
                                {{ $difficultyLabel }}
                            </span>
                        </div>

                        <p class="text-gray-700 leading-relaxed">
                            {{ $challenge->description }}
                        </p>

                        <p class="mt-3 font-semibold text-gray-900">
                            Duur:
                            <span class="font-normal">
                                {{ $challenge->duration }}
                            </span>
                        </p>

                    </div>
                </div>
            </section>

            {{-- Stappen --}}
            <section class="rounded-2xl shadow-md md:p-8 bg-blue-200"
                     aria-labelledby="steps-heading">

                @if($challenge->steps->isNotEmpty())
                    <h2 id="steps-heading"
                        class="text-xl font-bold text-gray-900 mb-4">
                        Stappen
                    </h2>

                    <ul class="space-y-3" aria-label="Stappen van deze uitdaging">
                        @foreach($challenge->steps as $step)
                            <li class="flex gap-3 items-start">

                                <input
                                    type="checkbox"
                                    class="mt-1 w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500"
                                    aria-label="Stap {{ $step->step_number }} voltooien"
                                >

                                <label class="leading-relaxed cursor-pointer">
                                    <span class="font-semibold text-gray-900">
                                        {{ $step->step_number }}.
                                    </span>
                                    {!! nl2br(e($step->step_description)) !!}
                                </label>

                            </li>
                        @endforeach
                    </ul>
                @endif

                @php
                    $user = auth()->user();
                    $hasBadge = false;

                    if ($user && $challenge->badge_id) {
                        $hasBadge = $user->badges->contains('id', $challenge->badge_id);
                    }
                @endphp

                @auth
                    @if($hasBadge)
                        <div class="flex flex-col sm:flex-row items-center gap-4 mt-6">

                            <img
                                src="{{ asset($challenge->badge->image) }}"
                                alt="Badge {{ $challenge->badge->name }}"
                                class="w-20 h-20"
                            >

                            <div>
                                <p class="font-semibold">
                                    Je hebt deze badge al verdiend!
                                </p>
                                <p>
                                    {{ $challenge->badge->name }}
                                </p>
                            </div>

                            <a href="{{ route('dashboard') }}"
                               class="inline-block bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-6 py-3 rounded-lg shadow-lg transition">
                                Ga terug
                            </a>

                        </div>
                    @else
                        <div class="mt-8 text-center">
                            <a href="{{ route('upload.show', $challenge) }}"
                               class="inline-block bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-6 py-3 rounded-lg shadow-lg transition">
                                Lever nu in
                            </a>
                        </div>
                    @endif
                @endauth

            </section>

        </div>
    </main>

</x-app-layout>
